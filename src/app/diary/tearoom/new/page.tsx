...
export default function NewTearoomVisitPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('tearoom_visits').insert({
        user_id: user.id,
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-4 py-8">
        <div className="space-y-8">
          ...
          <div className="flex gap-4 pb-8">
            ...
            <Link href="/diary" className="flex-1 btn-secondary py-3 text-center">
              취소
            </Link>
          </div>
        </div>
